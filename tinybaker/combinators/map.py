...
def map_tags(base_step: StepDefinition, input_mapping={}, output_mapping={}):
  ...
  if len(extra_input_keys) > 0:
    msg = "Unexpected key(s) for input mapping: {}".format(", ".join(extra_input_keys))
    raise BakerError(msg)

  extra_output_keys = set(output_mapping) - base_step.output_file_set
  if len(extra_output_keys) > 0:
    msg = "Unexpected key(s) for output mapping: {}".format(", ".join(extra_output_keys))
    raise BakerError(msg)

  mapping_input_file_set = _map_names(base_step.input_file_set, _invert_mapping(input_mapping))
  ...
  class TagMapping(StepDefinition):
    ...
    def script(self):
      ...
      input_paths = _map_filerefs_to_new_paths(self.input_files, self._input_mapping)
      output_paths = _map_filerefs_to_new_paths(self.output_files, _invert_mapping(self._output_mapping))
