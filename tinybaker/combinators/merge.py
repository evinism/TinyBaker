...
def merge(merge_steps: List[StepDefinition]):
  ...
  if len(merge_output_file_set) != sum([len(step.output_file_set) for step in merge_steps]):
  ...
  class Merged(StepDefinition):
    ...
    def script(self):
      merge_input_paths = {tag: self.input_files[tag].path for tag in self.input_files}
      merge_output_paths = {tag: self.output_files[tag].path for tag in self.output_files}

      instances = []

      for step in self.steps:
        input_files = {tag: merge_input_paths[tag] for tag in merge_input_paths if tag in step.input_file_set}
        output_files = {tag: merge_output_paths[tag] for tag in merge_output_paths if tag in step.output_file_set}
