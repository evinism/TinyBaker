...
def sequence(seq_steps: List[StepDefinition]):
  ...
  for i in range(len(seq_steps) - 2):
    ...
    if len(set.intersection(unconsumed_outputs, additional_outputs)):
      ...
      raise TagConflictError("Multiple steps in sequence generate output tag {}".format(", ".join(items)))
    ...
    if len(set.intersection(unconsumed_outputs, additional_outputs)):
      items = set.intersection(unconsumed_outputs, additional_outputs)
      raise TagConflictError("Multiple steps in sequence expect input tag {}".format(", ".join(items)))
  ...
  class Sequence(StepDefinition):
    ...
    def script(self):
      ...
      seq_input_paths = {tag: self.input_files[tag].path for tag in self.input_files}
      seq_output_paths = {tag: self.output_files[tag].path for tag in self.output_files}

      prev_output_paths = {}
      for step in self.steps:
        # Step input files
        input_paths_from_sequence = {tag: seq_input_paths[tag] for tag in seq_input_paths if tag in step.input_file_set}
        input_paths_from_prev = {tag: prev_output_paths[tag] for tag in prev_output_paths if tag in step.input_file_set}
        input_paths = {}
        input_paths.update(input_paths_from_sequence)
        input_paths.update(input_paths_from_prev)

        # Generate output fileset
        generated_output_paths = {tag: self._generate_temp_filename(sequence_instance_id) for tag in step.output_file_set - self.output_file_set}
        output_paths_from_sequence = {tag: seq_output_paths[tag] for tag in seq_output_paths if tag in step.output_file_set}
        output_paths = {}
        output_paths.update(generated_output_paths)
        output_paths.update(output_paths_from_sequence)

        instances.append(step(input_paths=input_paths, output_paths=output_paths))
